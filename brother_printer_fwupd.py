...
PrinterInfo = namedtuple('PrinterInfo', ('model', 'serial', 'spec', 'fw_versions'))
    ...
    for error_indication, error_status, error_index, var_binds in nextCmd(
        ...
        if error_indication:
            ...
            if name == 'MODEL':
                ...
                firmver = value
                fw_versions.append((firmid, firmver))
                firmid = None
                firmver = None
...
'''
    ...
    resp = requests.post(FW_UPDATE_URL, data=api_data, headers={'Content-Type': 'text/xml'})
...
def main():
    ...
    print(f'[i]   Detected {printer_info.model} with {len(printer_info.fw_versions)} firmware parts.')
